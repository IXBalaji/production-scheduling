...
const MachineStatus = () => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-200">
      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Machine Status</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {machines.map((machine) => (
          <div key={machine.id} className="border border-gray-200 rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                {getStatusIcon(machine.status)}
                <h4 className="text-sm sm:text-base font-medium text-gray-900">{machine.name}</h4>
              </div>
            ...
            <div className="space-y-2">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-xs sm:text-sm font-medium">{machine.utilization}%</span>
                </div>
