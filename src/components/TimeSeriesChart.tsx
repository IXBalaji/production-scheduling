  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Production Trends & AI Predictions</h3>
          ...
          {productionData.map((data, index) => (
            <div key={index} className="flex-1 flex flex-col items-center">
              <div className="w-full flex flex-col items-center space-y-1">
                {/* Efficiency Bar */}
                <div className={`w-4 bg-gray-200 rounded-t ${data.isPrediction ? 'opacity-70' : ''}`}>
                  <div 
                    className={`${data.isPrediction ? 'bg-purple-500' : 'bg-blue-500'} rounded-t transition-all duration-300`}
                    style={{ height: `${(data.efficiency / maxEfficiency) * 100}px` }}
                  ></div>
                </div>
                ...
                <div className={`w-4 bg-gray-200 rounded-t ${data.isPrediction ? 'opacity-70' : ''}`}>
                  <div 
                    ...
                    style={{ height: `${(data.output / maxOutput) * 100}px` }}
                  ></div>
                </div>
              </div>
              
              <span className={`text-xs mt-2 ${data.isPrediction ? 'text-purple-500 font-medium' : 'text-gray-500'}`}>
                {data.hour}
              </span>
            </div>
          ))}
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600">Avg Efficiency: {avgEfficiency.toFixed(1)}%</span>
        </div>
        <div className="flex items-center space-x-2">
          <TrendingUp className="h-4 w-4 text-green-500" />
          <span className="text-sm text-gray-600">Total Output: {totalOutput} units</span>
        </div>
      </div>
    </div>
  );
